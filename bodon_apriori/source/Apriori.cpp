...
//      getchar();
      trie_size=trie->node_number();
      trie->statistics();
      cout<<endl<<"Generating "<<candidate_size+1<<"-itemset candidates!"<<endl;
      trie->candidate_generation(candidate_size);
      trie->statistics();
//      trie->show_content();
//      getchar();
   ...
   ofstream frequence_file("../frequence.txt");
   if (min_conf)
   {
        cout<<"\nGenerating association rules...!\n";
      trie->association(outcomefile, frequence_file, min_conf);
   }
     cout<<"\nMining is done!\n";

   frequence_file.close();
