...
void Trie_hash::assoc_rule_assist( ofstream& outcomefile, ofstream& frequence_file,const double min_conf,
                                   unsigned long actual_state, set<itemtype>& consequence_part) const
{
   if( type_vector[actual_state] ) Trie::assoc_rule_assist( outcomefile,frequence_file, min_conf, actual_state,consequence_part );
   else
   {
      ...
      for( edge_index = 1; edge_index < hash_modulus; edge_index++)
         if( statearray[actual_state][edge_index] )
         {
            ...
            assoc_rule_assist( outcomefile, frequence_file, min_conf, statearray[actual_state][edge_index], consequence_part);
...
void Trie_hash::show_content() const
{
   ...
   {
      cout << '\n' << stateIndex << "-id �llapot m�rete:" << itemarray[stateIndex].size()
           << " sz�ml�l�ja: " << countervector[stateIndex] << " leghosszab utja: "<<maxpath[stateIndex]
           << ", ra mut. �llapot" << parent[stateIndex]<<" ,tipusa:";
      if( type_vector[stateIndex] )
      {
         cout << " norm�lis cs�cs, levelei:";
