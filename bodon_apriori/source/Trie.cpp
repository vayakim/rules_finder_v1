...
void Trie::assoc_rule_find( ofstream& outcomefile, ofstream& frequence_file, const double min_conf, set<itemtype>& condition_part,
                            set<itemtype>& consequence_part, const unsigned long union_support) const
{
   ...
   if( condition_part.empty() || *(--condition_part.end()) < *item_it)
   {
      ...
      if( union_support > countervector[is_included(condition_part)] * min_conf)
      {
         ...
         outcomefile << orderarray[*item_it_2]-1;
         outcomefile << endl;
         frequence_file << union_support << "," <<((double) union_support) / countervector[is_included(condition_part)] << endl;
      }
      
      else if( consequence_part.size() > 1 ) assoc_rule_find( outcomefile, frequence_file, min_conf, condition_part, consequence_part, union_support );
      item_it = (consequence_part.insert( item )).first;
      condition_part.erase( item );
   }
}

void Trie::assoc_rule_assist( ofstream& outcomefile, ofstream& frequence_file, const double min_conf, unsigned long actual_state, set<itemtype>& consequence_part) const
{
   if( consequence_part.size() > 1 )
   {
      set<itemtype> condition_part;
      assoc_rule_find( outcomefile, frequence_file, min_conf, condition_part, consequence_part, countervector[actual_state] );
   }
   ...
   for( vector<itemtype>::const_iterator it_item = itemarray[actual_state].begin(); it_item != itemarray[actual_state].end(); it_item++, it_state++)
   {
      ...
      assoc_rule_assist( outcomefile, frequence_file, min_conf, *it_state, consequence_part);
...
*/
void Trie::association( ofstream& outcomefile, ofstream& frequence_file, const double min_conf ) const
{

   set<itemtype> consequence_part;
   assoc_rule_assist( outcomefile, frequence_file, min_conf, 0, consequence_part );
}
/**
  \param basket The given basket.
*/
